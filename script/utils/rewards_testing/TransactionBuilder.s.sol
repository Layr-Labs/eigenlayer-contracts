 ...
 */
contract TransactionBuilder is ZeusScript {
    ...
    uint256 numOperatorsRegisteredToAVSs = 80;
    ...
    function seedOperators() external parseState {
        ...
        for(uint256 i = 0; i < operators.length; i += batchSize){
            ...
            address(transactionSubmitter).call{value: amountToSend * batch.length}("");
    ...
    function registerOperators() external parseState {
        ...
        for(uint256 i = operatorsRegistered; i < maxOperatorIndex; i++){
            ...
            _delegationManager().registerAsOperator(operatorDetails, "");
            vm.stopBroadcast();
    ...
    function deployTransactionSubmitter() external parseState {
        ...
        TransactionSubmitter transactionSubmitterImpl = new TransactionSubmitter(
            ...
            _avsDirectory(),
            _strategyManager(),
            IRewardsCoordinator(zDeployedProxy(type(RewardsCoordinator).name)),
            _strategyFactory()
        );
    ...
    function undelegateStakers(uint32 startIndex, uint32 endIndex) external parseState {
        for (uint32 i = startIndex; i <= endIndex; i++) {
            ...
            if (!_delegationManager().isDelegated(staker)) {
                continue;
            }
    ...
    function delegateSecondHalfOfStakers() external parseState {
        ...
        for (uint256 i = firstStaker; i < lastStaker; i += batchSize) {
            ...
            for (uint256 j = 0; j < batchSize; j++) {
                ...
                if(!_delegationManager().isOperator(operator)){
    ...
    function depositStakersMultiStrat(uint256 startStaker, uint256 endStaker) external parseState {
        ...
        for(uint256 i = startStaker; i < endStaker; i += batchSize) {
            ...
            for(uint256 j = 0; j < batchSize; j++){
                ...
                uint256 nonce = StrategyManager(address(_strategyManager())).nonces(staker);
    ...
    function delegateByIndex(uint32 stakerIndex, uint32 operatorIndex) public parseState {
        ...
        vm.startBroadcast(staker);
        _delegationManager().delegateTo(operator, approverSignatureAndExpiry, "");
        vm.stopBroadcast();
    }
    ...
    function _getOperatorSignature(
        ...
        {
            bytes32 digestHash = _avsDirectory().calculateOperatorAVSRegistrationDigestHash(operator, avs, operatorSignature.salt, operatorSignature.expiry);
    ...
    function _getStakerDelegationSignature(
        ...
        {
            bytes32 digestHash = _delegationManager().calculateCurrentStakerDelegationDigestHash(staker, operator, stakerSignature.expiry);
    ...
    function _getStakerStrategyInfo(
        ...
        return _getStakerStrategyInfoManualNonce(stakerPrivateKey, staker, strategy, token, amount, StrategyManager(address(_strategyManager())).nonces(staker));
    }
    ...
    function _delegationManager() internal view returns (IDelegationManager) {
        return IDelegationManager(zDeployedProxy(type(DelegationManager).name));
    }

    function _strategyManager() internal view returns (IStrategyManager) {
        return IStrategyManager(zDeployedProxy(type(StrategyManager).name));
    }

    function _avsDirectory() internal view returns (IAVSDirectory) {
        return IAVSDirectory(zDeployedProxy(type(AVSDirectory).name));
    }

    function _strategyFactory() internal returns (IStrategyFactory) {
        return IStrategyFactory(zDeployedProxy(type(StrategyFactory).name));
    }
    ...
    function setWhitelistedStrategies() external parseState {
        ...
        vm.startBroadcast();
        _strategyFactory().whitelistStrategies(strategies, thirdPartyTransfersForbiddenValues);
        vm.stopBroadcast();
    }
