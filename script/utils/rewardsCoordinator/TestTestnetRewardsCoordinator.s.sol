 ...
 *
 */
contract TestTestnetRewardsCoordinator is ExistingDeploymentParser {
    // 0-20: operators
    // 21-100: stakers
    ...
    IStrategy wethStrategy = IStrategy(0x80528D6e9A2BAbFc766965E0E26d5aB08D9CFaF9);
    IERC20 eigen = IERC20(0x3B78576F7D6837500bA3De27A60c7f594934027E);
    ...
    function _setupScript() internal virtual {
        ...
        _parseDeployedContracts("script/output/holesky/M2_deploy_testnet.output.json");
        ...
        // );
        require(
            address(eigenStrategy) == 0x43252609bff8a13dFe5e057097f2f45A24387a84,
            "Eigen strategy address incorrect"
        );
        require(
            address(wethStrategy) == 0x80528D6e9A2BAbFc766965E0E26d5aB08D9CFaF9,
    ...
    /**
        ...
        ========ANVIL========
        forge script script/utils/rewardsCoordinator/TestTestnetRewardsCoordinator.s.sol:TestTestnetRewardsCoordinator \
    ...
    /**
        ...
        forge script script/utils/rewardsCoordinator/TestTestnetRewardsCoordinator.s.sol:TestTestnetRewardsCoordinator \
            ...
            --sig "createAVSRewardsSubmission(string memory startTimestampType, address avsServiceManager, address paymentTokenAddress, uint256 amount)" \
            "current" 0x7c79d23c1fdddc7075f55dd357610a3cc69e73a0 0x0000000000000000000000000000000000000000 1000000000000000000000000000000
     */
    ...
    function depositWETHAndEIGEN(uint8 numStakers) external virtual {
        ...
        for (uint256 i = 71; i < 72 + numStakers; ++i) {
            (address staker, /* privateKey */) = deriveRememberKey(TEST_MNEMONIC, uint32(i));

            // Transfer strategy token to staker
            vm.startBroadcast();
            uint256 depositAmount = 2e16;
    ...
    function registerOperators(uint8 numOperators, address serviceManagerAddress) external virtual {
        ...
        for (uint256 i = 5; i < numOperators; ++i) {
    ...
    /**
        ...
        forge script script/utils/rewardsCoordinator/TestTestnetRewardsCoordinator.s.sol:TestTestnetRewardsCoordinator \
            --rpc-url $RPC_HOLESKY --private-key $PRIVATE_KEY --broadcast -vvvv \
            --sig "delegateTo(uint8 stakerId, uint8 operatorId)" 71 0

        forge script script/utils/rewardsCoordinator/TestTestnetRewardsCoordinator.s.sol:TestTestnetRewardsCoordinator \
            --rpc-url $RPC_HOLESKY --private-key $PRIVATE_KEY --broadcast -vvvv \
            --sig "delegateMultiple(uint8 numStakers, uint8 operatorId)" 3 0
     */

    function delegateTo(uint8 stakerId, uint8 operatorId) public virtual {
        _setupScript();
    ...
    function delegateMultiple(uint8 numStakers, uint8 operatorId) external {
        _setupScript();
        operatorId = 9;
        uint8 maxStakers = 70;
        for (uint8 i = 65; i < maxStakers; i++) {
            (address staker, /* privateKey */) = deriveRememberKey(TEST_MNEMONIC, uint32(i));
            (address operator, /* privateKey */) = deriveRememberKey(TEST_MNEMONIC, uint32(operatorId));
