...
contract PopulateSRC is Script, Test, ExistingDeploymentParser {
    ...
    uint32 constant NUM_OPERATORS_PER_OPSET = 20;
    uint32 constant NUM_STRATS_PER_OPSET = 2;
    uint256 constant TOKEN_AMOUNT_PER_OPERATOR = 1 ether;

    
    function run(string memory outputFile, string memory strategyFile) public {
        _parseDeployedContracts(outputFile);
        ...
        vm.broadcast();
        OperatorFactory operatorFactory = new OperatorFactory(delegationManager, strategyManager, allocationManager);
...
contract OperatorFactory is Test {
    ...
    constructor(IDelegationManager _delegationManager, IStrategyManager _strategyManager, IAllocationManager _allocationManager) {
    ...
    function createManyOperators(uint256 numOperatorsPerOpset) public returns(address[] memory) {
        ...
        for (uint256 i = 0; i < operators.length; ++i) {
            operators[i] = address(new Operator(delegationManager, allocationManager));
        }
        return operators;
    }
    ...
    function allocateForOperators(IStrategy strategy, OperatorSet calldata operatorSet, address[] memory operators, uint64 magnitudeForOperators) public {
        ...
        IAllocationManager.MagnitudeAllocation[] memory allocations = new IAllocationManager.MagnitudeAllocation[](1);
        allocations[0] = IAllocationManager.MagnitudeAllocation({
            strategy: strategy,
...
contract Operator is IERC1271 {
    constructor(IDelegationManager delegationManager, IAllocationManager allocationManager) {
