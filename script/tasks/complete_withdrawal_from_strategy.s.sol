...
contract CompleteWithdrawFromStrategy is Script, Test {
    ...
    function run(
        ...
        IDelegationManagerTypes.Withdrawal memory withdrawal =
            getWithdrawalStruct(strategy, sf, deposits, nonce, startBlock);
    ...
    function getWithdrawalStruct(
        ...
        DepositScalingFactor memory dsf = DepositScalingFactor(sf);

        // Get scaled shares for the given amount
        uint256[] memory scaledShares = new uint256[](1);
        scaledShares[0] = dsf.scaleForQueueWithdrawal(deposits);

        // Log the current state before completing
        emit log_uint(sf);
        emit log_uint(deposits);
