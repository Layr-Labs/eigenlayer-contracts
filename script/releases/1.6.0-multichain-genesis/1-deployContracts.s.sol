...
contract DeployDestinationGenesis is EOADeployer {
    ...
    function _runAsEOA() internal override {
        ...
        address[] memory opsMultisigInitialOwners = new address[](6);
        opsMultisigInitialOwners[0] = Env.multichainDeployerMultisig();
        for (uint256 i = 1; i < 10; i++) {
            opsMultisigInitialOwners[i] = address(0);
        }

        // Setup pauser multisig initial owners
        address[] memory pauserMultisigInitialOwners = new address[](10);
        pauserMultisigInitialOwners[0] = Env.multichainDeployerMultisig();
        for (uint256 i = 1; i < 10; i++) {
            pauserMultisigInitialOwners[i] = address(0);
        }
    ...
    function _getMultisigOwner(string memory path) internal view returns (address[] memory) {
        // Read the TOML file
