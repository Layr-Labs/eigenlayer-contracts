...
abstract contract IntegrationDeployer is ExistingDeploymentParser {
    ...
    function _configRand(uint24 _randomSeed, uint256 _assetTypes, uint256 _userTypes) internal {
        ...
        for (uint256 i = 0; i < assetTypes.length; i++) {
            emit log(assetTypeToStr[uint256(uint8(assetTypes[i]))]);
        }

        emit log("_configRand: these User contracts will be initialized:");
        for (uint256 i = 0; i < userTypes.length; i++) {
            emit log(userTypeToStr[uint256(uint8(userTypes[i]))]);
        }
    ...
    function _genRandUser(string memory name, uint256 userType) internal returns (User) {
