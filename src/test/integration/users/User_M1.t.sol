...
contract User_M1 is User {
    ...
    function _createPod() internal virtual override {
        IEigenPodManager_DeprecatedM1(address(deployer.eigenPodManager())).createPod();
        // get EigenPod address
        pod = EigenPod(payable(address(IEigenPodManager_DeprecatedM1(address(deployer.eigenPodManager())).ownerToPod(address(this)))));
    }
}
    ...
    function depositIntoEigenlayer_M1(IStrategy[] memory strategies, uint[] memory tokenBalances) public override createSnapshot {
        ...
        for (uint i = 0; i < strategies.length; i++) {
            ...
            bytes32 structHash = keccak256(
                abi.encode(deployer.strategyManager().DEPOSIT_TYPEHASH(), strat, underlyingToken, tokenBalance, nonceBefore, expiry)
            );
            bytes32 domain_separator = deployer.strategyManager().domainSeparator();
