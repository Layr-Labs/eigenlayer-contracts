...
contract User is PrintUtils {
    ...
    function undelegate() public virtual createSnapshot returns (IDelegationManager.Withdrawal[] memory) {
        _logM("undelegate");

        IDelegationManager.Withdrawal[] memory expectedWithdrawals =
            _getExpectedWithdrawalStructsForStaker(address(this));
    ...
    function forceUndelegate(
        User staker
    ) public virtual createSnapshot returns (IDelegationManager.Withdrawal[] memory) {
        _logM("forceUndelegate", staker.NAME());

        IDelegationManager.Withdrawal[] memory expectedWithdrawals =
            _getExpectedWithdrawalStructsForStaker(address(staker));
    ...
    function queueWithdrawals(
        ...
        params[0] =
            IDelegationManager.QueuedWithdrawalParams({strategies: strategies, shares: shares, withdrawer: withdrawer});
    ...
    function _getExpectedWithdrawalStructsForStaker(
        ...
        IDelegationManager.Withdrawal[] memory expectedWithdrawals =
            new IDelegationManager.Withdrawal[](strategies.length);
...
contract User_AltMethods is User {
    ...
    function delegateTo(
        ...
        bytes32 digestHash =
            delegationManager.calculateCurrentStakerDelegationDigestHash(address(this), address(operator), expiry);
        ...
        delegationManager.delegateToBySignature(
            address(this), address(operator), stakerSignatureAndExpiry, emptySig, bytes32(0)
        );
