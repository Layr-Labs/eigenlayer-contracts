...
abstract contract IntegrationBase is IntegrationDeployer {
    ...
    function _sendRandomETH(
        address destination
    ) internal returns (uint64 gweiSent, uint remainderSent) {
    ...
    function assert_Snap_Allocations_Modified(
        ...
        IAllocationManagerTypes.Allocation[] memory curAllocs =
            _getAllocations(operator, allocateParams.operatorSet, allocateParams.strategies);
        IAllocationManagerTypes.Allocation[] memory prevAllocs =
            _getPrevAllocations(operator, allocateParams.operatorSet, allocateParams.strategies);
        ...
        for (uint i = 0; i < allocateParams.strategies.length; i++) {
            ...
            if (completed) {
                ...
                assertEq(curAlloc.currentMagnitude, prevAlloc.currentMagnitude, string.concat(err, " (currentMagnitude)"));
                assertEq(
            ...
            if (prevAlloc.currentMagnitude < allocateParams.newMagnitudes[i]) {
                uint delta = allocateParams.newMagnitudes[i] - prevAlloc.currentMagnitude;

                // FIXME: Fails on `testFuzz_deposit_delegate_allocate` when completed == true.
                if (!completed) {
                    assertEq(curMagnitude.encumbered, prevMagnitude.encumbered + delta, string.concat(err, " (encumbered magnitude)"));

                    assertEq(curMagnitude.allocatable, prevMagnitude.allocatable - delta, string.concat(err, " (allocatable magnitude)"));
                }
            } else {
                uint delta = prevAlloc.currentMagnitude - allocateParams.newMagnitudes[i];

                // if deallocated, and completed
                if (completed) {
                    assertEq(curMagnitude.encumbered - delta, prevMagnitude.encumbered, string.concat(err, " (encumbered magnitude)"));

                    assertEq(curMagnitude.allocatable, prevMagnitude.allocatable + delta, string.concat(err, " (allocatable magnitude)"));
                } else {
                    assertEq(curMagnitude.encumbered, prevMagnitude.encumbered, string.concat(err, " (encumbered magnitude)"));
            ...
            assertEq(curMagnitude.max, prevMagnitude.max, string.concat(err, " (max magnitude)"));
        }
    }
    ...
    function assert_HasUnderlyingTokenBalances_AfterSlash(
        ...
        for (uint i; i < allocateParams.strategies.length; ++i) {
            ...
            uint balance = strat == BEACONCHAIN_ETH_STRAT ? address(staker).balance : strat.underlyingToken().balanceOf(address(staker));

            uint maxDelta = strat == BEACONCHAIN_ETH_STRAT ? 1 gwei : 3;
            ...
            if (slashingParams.strategies.contains(strat)) {
                ...
                expectedBalances[i] -= expectedBalances[i].mulWadRoundUp(allocateParams.newMagnitudes[i].mulWadRoundUp(wadToSlash));
            }
    ...
    function assert_Snap_Added_Staker_DepositShares(User staker, IStrategy strat, uint _addedShares, string memory err) internal {
    ...
    function assert_Snap_Removed_StakerDepositShares(User staker, IStrategy strat, uint _removedShares, string memory err) internal {
        IStrategy[] memory strategies = new IStrategy[](1);
    ...
    function assert_Snap_Removed_StakerWithdrawableShares(User staker, IStrategy strat, uint _removedShares, string memory err) internal {
        IStrategy[] memory strategies = new IStrategy[](1);
        uint[] memory removedShares = new uint[](1);
        strategies[0] = strat;
        removedShares[0] = _removedShares;
    ...
    /// since the last snapshot
    function assert_Snap_Unchanged_StakerDepositShares(User staker, string memory err) internal {
        IStrategy[] memory strategies = allStrats;
    ...
    function _getBeaconChainSlashingFactor(
        User staker
