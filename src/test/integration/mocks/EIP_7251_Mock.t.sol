...
contract EIP_7251_Mock {
    ...
    uint MAX_CONSOLIDATION_REQUESTS_PER_BLOCK = 2;
    uint TARGET_CONSOLIDATION_REQUESTS_PER_BLOCK = 1;
    ...
    uint queueHead;
    uint queueTail;
    mapping(uint => ConsolidationRequest) requestQueue;
    ...
    function _doAddRequest() internal {
        uint fee = _getFee();
        require(msg.value >= fee, "EIP_7251_Mock: insuffient value for fee");
        ...
        ConsolidationRequest storage request = requestQueue[queueTail];
    ...
    function _doReadRequests() internal {
        ConsolidationRequest[] memory reqs = _dequeueConsolidationRequests();
    ...
    function _dequeueConsolidationRequests() internal returns (ConsolidationRequest[] memory reqs) {
        uint numInQueue = queueTail - queueHead;
        uint numToDequeue = 
            numInQueue > MAX_CONSOLIDATION_REQUESTS_PER_BLOCK ? MAX_CONSOLIDATION_REQUESTS_PER_BLOCK : numInQueue;

        reqs = new ConsolidationRequest[](numToDequeue);
