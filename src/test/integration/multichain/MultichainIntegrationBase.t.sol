...
abstract contract MultichainIntegrationBase is IntegrationBase {
    ...
    function _deployMultichainContracts() internal {
        ...
        operatorTableUpdaterImplementation =
            new OperatorTableUpdater(IBN254CertificateVerifier(address(0)), IECDSACertificateVerifier(address(0)), "1.0.0");
    ...
    function _hashBN254Table(IBN254TableCalculatorTypes.BN254OperatorSetInfo memory operatorSetInfo) internal pure returns (bytes32) {
        return keccak256(abi.encode(operatorSetInfo));
    }

    /// @dev Calculate hash of ECDSA operator table
    function _hashECDSATable(IECDSATableCalculatorTypes.ECDSAOperatorInfo[] memory operatorInfos) internal pure returns (bytes32) {
    ...
    function _updateGlobalTableRoot(bytes32 globalTableRoot, uint32 confirmationTimestamp) internal {
        ...
        OperatorSet memory globalRootConfirmerSet = operatorTableUpdater.getGlobalRootConfirmerSet();

        // Get the global root confirmer set's reference timestamp (from initialization)
        uint32 globalRootConfirmerTimestamp = operatorTableUpdater.getGlobalConfirmerSetReferenceTimestamp();
