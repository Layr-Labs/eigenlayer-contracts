...
contract Integration_Pectra_Features_Base is IntegrationCheckUtils {
    ...
    function testFuzz_consolidate(uint24 _r) public rand(_r) {
        ...
        // Deal ETH and start 2 validators; both ETH
        (uint40[] memory validatorsA, uint64 balanceGweiA,) = staker.startValidators(1);
        (uint40[] memory validatorsB, uint64 balanceGweiB,) = staker.startValidators(1);

        uint40[] memory validators = _zip(validatorsA, validatorsB);
        uint balanceWei = (balanceGweiA + balanceGweiB) * GWEI_TO_WEI;

        _printValidators(validators);

        staker.verifyWithdrawalCredentials(validators);
        check_Deposit_State(staker, beaconChainETHStrategy.toArray(), balanceWei.toArrayU256());
        ...
        // check_CompleteCheckpoint_WithExits_State(staker, consolidated, 0); TODO - yet another checkpointBalanceExited edge case

        _printValidators(newValidators);
