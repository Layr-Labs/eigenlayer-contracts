...
contract IntegrationTestUtils is IntegrationBase {
    ...
    function assertDepositState(User staker, IStrategy[] memory strategies, uint[] memory shares) internal {
    ...
    function assertDelegationState(User staker, User operator, IStrategy[] memory strategies, uint[] memory shares) internal {
    ...
    function assertQueuedWithdrawalState(User staker, User operator, IStrategy[] memory strategies, uint[] memory shares, IDelegationManager.Withdrawal[] memory withdrawals, bytes32[] memory withdrawalRoots) internal {
    ...
    function assertUndelegateState(
        ...
        assertEq(withdrawals.length, withdrawalRoots.length, "withdrawals and roots should have same length");
        ...
        assert_AllWithdrawalsPending(withdrawalRoots, "staker's withdrawal should now be pending");
        assert_Snap_Added_QueuedWithdrawal(staker, withdrawal, "staker should have increased nonce by 1");
        assert_Snap_Removed_OperatorShares(operator, strategies, shares, "failed to remove operator shares");
        assert_Snap_Removed_StakerShares(staker, strategies, shares, "failed to remove staker shares");
    }
