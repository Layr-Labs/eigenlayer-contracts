...
contract Integration_Deposit_Delegate_Redelegate_Complete is IntegrationCheckUtils {
    ...
    function testFuzz_deposit_delegate_reDelegate_completeAsTokens(
        ...
        for (uint256 i = 0; i < withdrawals.length; ++i) {
            ...
            check_Withdrawal_AsShares_Undelegated_State(
                staker, operator1, withdrawals[i], withdrawals[i].strategies, withdrawals[i].shares
            );
        }
        ...
        assertNotEq(
            ...
            "staker should not be delegated to operator1"
        );
    ...
    function testFuzz_deposit_delegate_reDelegate_completeAsShares(
        ...
        for (uint256 i = 0; i < withdrawals.length - 1; i++) {
            IERC20[] memory tokens = staker.completeWithdrawalAsTokens(withdrawals[i]);
        ...
        uint256[] memory finalExpectedTokens = _calculateExpectedTokens(strategies, shares);
        check_Withdrawal_AsTokens_State(
            staker,
            operator2,
    ...
    function testFuzz_deposit_delegate_reDelegate_depositAfterRedelegate(
...

        {
            ...
            uint256[] memory numTokensToDeposit = new uint256[](tokenBalances.length);
            ...
            uint256[] memory halfShares = _calculateExpectedShares(strategies, numTokensToDeposit);
            ...
            uint256[] memory sharesAdded = _calculateExpectedShares(strategies, numTokensRemaining);
...

        {
            ...
            // Complete withdrawals
            for (uint256 i = 0; i < newWithdrawals.length; i++) {
                uint256[] memory expectedTokens =
                    _calculateExpectedTokens(newWithdrawals[i].strategies, newWithdrawals[i].shares);
                IERC20[] memory tokens = staker.completeWithdrawalAsTokens(newWithdrawals[i]);
                check_Withdrawal_AsTokens_State(
                    staker, operator2, newWithdrawals[i], strategies, shares, tokens, expectedTokens
                );
            }
        }
    }
    ...
    function testFuzz_deposit_delegate_undelegate_withdrawAsTokens_reDelegate_completeAsTokens(
        ...
        /// 1. Deposit Into Strategies
        staker.depositIntoEigenlayer(strategies, tokenBalances);
        uint256[] memory withdrawnTokenBalances = _calculateExpectedTokens(strategies, shares);
        check_Deposit_State(staker, strategies, shares);
        ...
        for (uint256 i = 0; i < withdrawals.length; i++) {
            ...
            IERC20[] memory tokens = staker.completeWithdrawalAsTokens(withdrawals[i]);
            check_Withdrawal_AsTokens_State(
                staker, operator2, withdrawals[i], strategies, shares, tokens, expectedTokens
            );
        }
    }
