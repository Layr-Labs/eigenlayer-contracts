...
contract Integration_Deposit_Delegate_UpdateBalance is IntegrationCheckUtils {
    ...
    function testFuzz_deposit_delegate_updateBalance_completeAsTokens(
        uint24 _random
    ) public {
        _configRand({
            _randomSeed: _random,
        ...
        (int256[] memory tokenDeltas, int256[] memory stakerShareDeltas, int256[] memory operatorShareDeltas) =
        ...
        assert_Snap_Delta_StakerShares(
            staker, strategies, stakerShareDeltas, "staker should have applied deltas correctly"
        );
        assert_Snap_Delta_OperatorShares(
            operator, strategies, operatorShareDeltas, "operator should have applied deltas correctly"
        );
