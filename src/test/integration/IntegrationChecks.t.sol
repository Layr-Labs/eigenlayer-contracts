...
contract IntegrationCheckUtils is IntegrationBase {
    ...
    function check_VerifyWC_State(User staker, uint40[] memory validators, uint64 beaconBalanceGwei) internal {
        uint256 beaconBalanceWei = beaconBalanceGwei * GWEI_TO_WEI;
        assert_Snap_Added_StakerShares(
            staker, BEACONCHAIN_ETH_STRAT, beaconBalanceWei, "staker should have added shares to beacon chain strat"
        );
    ...
    function check_CompleteCheckpoint_State(
        ...
        assert_Snap_Added_PodBalanceToWithdrawable(
            staker, "pod balance should have been added to withdrawable restaked exec layer gwei"
        );
    }
    ...
    function check_CompleteCheckpoint_EarnOnBeacon_State(User staker, uint64 beaconBalanceAdded) internal {
        ...
        uint256 balanceAddedWei = beaconBalanceAdded * GWEI_TO_WEI;
        assert_Snap_Added_StakerShares(
    ...
    function check_CompleteCheckpoint_WithSlashing_State(
        ...
        assert_Snap_Removed_StakerShares(
            ...
            "should have reduced shares by slashed amount"
        );
        assert_Snap_Removed_ActiveValidatorCount(
            staker, slashedValidators.length, "should have decreased active validator count"
        );
    ...
    function check_CompleteCheckpoint_WithExits_State(
        ...
        assert_Snap_Added_BalanceExitedGwei(
            staker, exitedBalanceGwei, "should have attributed expected gwei to exited balance"
        );
        assert_Snap_Removed_ActiveValidatorCount(
            staker, exitedValidators.length, "should have decreased active validator count"
        );
     ...
     *
     */
    function check_Deposit_State(User staker, IStrategy[] memory strategies, uint256[] memory shares) internal {
        ...
        assert_Snap_Added_StakerShares(
            staker, strategies, shares, "staker should expect shares in each strategy after depositing"
        );
    }
    ...
    function check_Deposit_State_PartialDeposit(
        ...
        assert_HasUnderlyingTokenBalances(
            staker, strategies, tokenBalances, "staker should have transferred some underlying tokens"
        );
        assert_Snap_Added_StakerShares(
    ...
    function check_Delegation_State(
        ...
        assertEq(
            address(operator), delegationManager.delegatedTo(address(staker)), "staker should be delegated to operator"
        );
        assert_HasExpectedShares(
            staker, strategies, shares, "staker should still have expected shares after delegating"
        );
    ...
    function check_QueuedWithdrawal_State(
        ...
        //     reduced shares.
        assertEq(
            withdrawalRoots.length, 1, "check_QueuedWithdrawal_State: should only have 1 withdrawal root after queueing"
        );
        assert_AllWithdrawalsPending(
            withdrawalRoots, "check_QueuedWithdrawal_State: staker withdrawals should now be pending"
        );
        ...
        assert_Snap_Removed_OperatorShares(
            operator, strategies, shares, "check_QueuedWithdrawal_State: failed to remove operator shares"
        );
        assert_Snap_Removed_StakerShares(
            staker, strategies, shares, "check_QueuedWithdrawal_State: failed to remove staker shares"
        );
    }
    ...
    function check_Undelegate_State(
        ...
        assertFalse(
            delegationManager.isDelegated(address(staker)), "check_Undelegate_State: staker should not be delegated"
        );
        assert_ValidWithdrawalHashes(
            withdrawals, withdrawalRoots, "check_Undelegate_State: calculated withdrawl should match returned root"
        );
        assert_AllWithdrawalsPending(
            withdrawalRoots, "check_Undelegate_State: stakers withdrawal should now be pending"
        );
        assert_Snap_Added_QueuedWithdrawals(
            staker, withdrawals, "check_Undelegate_State: staker should have increased nonce by withdrawals.length"
        );
        assert_Snap_Removed_OperatorShares(
            operator, strategies, shares, "check_Undelegate_State: failed to remove operator shares"
        );
        assert_Snap_Removed_StakerShares(
            staker, strategies, shares, "check_Undelegate_State: failed to remove staker shares"
        );
    }
    ...
    function check_Withdrawal_AsShares_State(
        ...
        if (operator != User(User(payable(0)))) {
            if (operator != staker) {
                assert_Snap_Unchanged_TokenBalances(
                    User(operator), "operator should not have any change in underlying token balances"
                );
            }
            assert_Snap_Added_OperatorShares(
                User(operator), withdrawal.strategies, withdrawal.shares, "operator should have received shares"
            );
        }
    }
    ...
    function check_Withdrawal_AsShares_Undelegated_State(
        ...
        assert_Snap_Unchanged_TokenBalances(staker, "staker should not have any change in underlying token balances");
        assert_Snap_Unchanged_TokenBalances(
            operator, "operator should not have any change in underlying token balances"
        );
        assert_Snap_Added_StakerShares(staker, strategies, shares, "staker should have received expected shares");
