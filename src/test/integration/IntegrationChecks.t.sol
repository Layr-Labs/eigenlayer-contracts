...
contract IntegrationCheckUtils is IntegrationBase {
     ...
     *
     */
    function check_VerifyWC_State(User_M2 staker, uint40[] memory validators, uint64 beaconBalanceGwei) internal {
    ...
    function check_QueuedWithdrawal_State(
        ...
        assert_Snap_Removed_OperatorShares(operator, strategies, shares, "check_QueuedWithdrawal_State: failed to remove operator shares");
        assert_Snap_Removed_StakerDepositShares(staker, strategies, shares, "check_QueuedWithdrawal_State: failed to remove staker shares");
    }
    ...
    function check_Undelegate_State(
        ...
        assert_Snap_Removed_OperatorShares(operator, strategies, shares, "check_Undelegate_State: failed to remove operator shares");
        assert_Snap_Removed_StakerDepositShares(staker, strategies, shares, "check_Undelegate_State: failed to remove staker shares");
    }
    ...
    function check_Withdrawal_AsShares_State(
        ...
        if (operator != User(User(payable(0)))) {
            ...
            assert_Snap_Added_OperatorShares(
                operator, withdrawal.strategies, withdrawal.scaledShares, "operator should have received shares"
            );
        }
    }
    ...
    function check_Withdrawal_AsTokens_State_AfterSlash(
        ...
        for (uint i; i < withdrawal.strategies.length; i++) {
            ...
            if (slashingParams.strategies.contains(strat)) {
                ...
                expectedTokens[i] -= expectedTokens[i].mulWadRoundUp(allocateParams.newMagnitudes[i].mulWadRoundUp(wadToSlash));
