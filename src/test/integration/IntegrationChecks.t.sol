...
contract IntegrationCheckUtils is IntegrationBase {
    ...
    *******************************************************************************/
    ...
    function check_QueuedWithdrawal_State(
        ...
        for (uint i = 0; i < strategies.length; i++) {
            ...
            // For a partial withdrawal, the dsf should not be changed & the strategy should still be in the staker strategy list
                ...
                assert_StrategyInStakerStrategyList(staker, strategies[i], "check_QueuedWithdrawal_State: staker strategy list should contain strategy");
            }
        }
    }
    ...
    function check_Decreased_SlashableStake(
        ...
        for (uint i = 0; i < strategies.length; i++) {
            if (withdrawableShares[i] > 0) {
                ...
                for (uint j = 0; j < operatorSets.length; j++) {
                    if (allocations[j].currentMagnitude > 0) {
                        assert_Snap_StakeBecomeUnslashable(operator, operatorSets[j], strategies[i].toArray(), "allocated strategies should have minSlashableStake decreased");
                    }
                }
            }
        }
    ...
    function check_Withdrawal_AsTokens_SlashInQueue_State(
        ...
        // Special case BC ETH strategy for now
        for (uint i = 0; i < strategies.length; i++) {
            IStrategy strategy = strategies[i];
            if (strategy != beaconChainETHStrategy) {
                assert_Snap_Unchanged_DSF(staker, strategy.toArray(), "dsf should not be changed");
            }
        }
    }
