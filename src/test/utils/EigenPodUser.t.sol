...
contract EigenPodUser is PrintUtils {
    ...
    function startValidators() public virtual createSnapshot returns (uint40[] memory, uint256) {
    ...
    /// Note: This method also advances one epoch forward on the beacon chain, so that
    /// withdrawal credential proofs are generated for each validator.
    function _startValidators() internal returns (uint40[] memory, uint256) {
        ...
        uint256 totalBeaconBalance = address(this).balance - balanceWei;
