...
contract StrategyManagerMock is Test {
    ...
    function setStakerStrategyListLengthReturnValue(
        uint valueToSet
    ) public {
    ...
    function removeDepositShares(address staker, IStrategy strategy, uint sharesToRemove) external {
        uint strategyIndex = _getStrategyIndex(staker, strategy);
    ...
    function _getStrategyIndex(address staker, IStrategy strategy) internal view returns (uint) {
        ...
        if (strategyIndex == type(uint).max) {
