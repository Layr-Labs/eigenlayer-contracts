...
contract EigenPodUnitTests_OffchainPartialWithdrawalProofTests is EigenPodUnitTests, IEigenPodEvents { 
    address feeRecipient = address(123);
    ...
    function testFuzz_proofCallbackRequest_PartialWithdrawalSumEqualsAlreadyProvenSum(uint64 endTimestamp, uint64 sumOfPartialWithdrawalsClaimedGwei, uint64 provenAmount, uint64 fee) external {
        ...
        uint64[] memory provenAmountArray = new uint64[](1);
        provenAmountArray[0] = provenAmount;
        ...
        withdrawalTimestampArray[0] = eigenPod.mostRecentWithdrawalTimestamp();
        uint64[] memory endTimestampArray = new uint64[](1);
        endTimestampArray[0] = endTimestamp;
        uint64[] memory feeArray = new uint64[](1);
        feeArray[0] = fee;
        IEigenPodManager.Journal memory journal = IEigenPodManager.Journal(
            provenAmountArray,
            bytes32(0),
            eigenPodAddressArray,
            podOwnerArray,
            withdrawalTimestampArray,
            endTimestampArray,
            feeArray,
            uint64(0)
        );
