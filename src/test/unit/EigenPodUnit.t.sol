...
contract EigenPodUnitTests is EigenLayerUnitTestSetup, EigenPodPausingConstants, IEigenPodEvents {
    ...
    function setUp() public virtual override {
        ...
        // Deploy EigenPod
        podImplementation = new EigenPod(ethPOSDepositMock, IEigenPodManager(address(eigenPodManagerMock)), GENESIS_TIME_LOCAL);
    ...
    function _newEigenPodStaker(
        uint rand
    ) internal returns (EigenPodUser, uint) {
    ...
    ) internal view returns (IEigenPodTypes.VALIDATOR_STATUS[] memory) {
        EigenPod pod = staker.pod();
    ...
    function _getPrevValidatorStatuses(
        EigenPodUser staker,
        bytes32[] memory pubkeyHashes
    ) internal timewarp returns (IEigenPodTypes.VALIDATOR_STATUS[] memory) {
...
contract EigenPodHarnessSetup is EigenPodUnitTests {
    ...
    function setUp() public virtual override {
        ...
        eigenPodHarnessImplementation =
            new EigenPodHarness(ethPOSDepositMock, IEigenPodManager(address(eigenPodManagerMock)), GENESIS_TIME_LOCAL);
