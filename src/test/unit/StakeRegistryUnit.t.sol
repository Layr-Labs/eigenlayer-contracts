...
contract StakeRegistryTest is Test {
    bytes internal constant ONLY_REGISTRY_COORDINATOR =
    ...
    bytes internal constant ONLY_SERVICE_MANAGER =
        "VoteWeigherBase.onlyServiceManagerOwner: caller is not the owner of the serviceManager";
    bytes internal constant GREATEST_QUORUM_GT_QUORUM_COUNT =
        "StakeRegistry._registerOperator: greatest quorumNumber must be less than quorumCount";
    bytes internal constant GREATEST_QUORUM_GT_QUORUM_COUNT_DEREGISTER =
        "StakeRegistry._deregisterOperator: greatest quorumNumber must be less than quorumCount";
    bytes internal constant OPERATOR_INSUFFICIENT_STAKE_FOR_QUORUM =
        "StakeRegistry._registerOperator: Operator does not meet minimum stake requirement for quorum";
    bytes internal constant LENGTH_MISMATCH = "Registry._initialize: minimumStakeForQuorum length mismatch";
    address internal constant REGISTRY_COORDINATOR = address(uint160(uint256(keccak256("registryCoordinator"))));
    address internal constant SERVICE_MANAGER_OWNER = address(uint160(uint256(keccak256("serviceManagerOwner"))));
    address internal constant PAUSER = address(uint160(uint256(keccak256("pauser"))));
    address internal constant UNPAUSER = address(uint160(uint256(keccak256("unpauser"))));
    address internal constant DEFAULT_OPERATOR = address(uint160(uint256(keccak256("defaultOperator"))));
    bytes32 internal constant DEFAULT_OPERATOR_ID = keccak256("defaultOperatorId");
    ...
    DelegationManagerMock internal delegationMock;
    ...
    function testFuzz_RevertsIf_LessThanMinimumStakeForQuorum_RegisterOperator(uint96[] memory stakesForQuorum) public {
        vm.assume(stakesForQuorum.length > 0);

        // set the weights of the operator
        // stakeRegistry.setOperatorWeight()

        bytes memory quorumNumbers = new bytes(
            stakesForQuorum.length > MAX_QUORUMS_TO_REGISTER_FOR ? MAX_QUORUMS_TO_REGISTER_FOR : stakesForQuorum.length
        );
        for (uint i = 0; i < quorumNumbers.length; i++) {
            quorumNumbers[i] = bytes1(uint8(i));
        }

        stakesForQuorum[stakesForQuorum.length - 1] =
            stakeRegistry.minimumStakeForQuorum(uint8(quorumNumbers.length - 1)) -
            1;
    ...
    function testFuzz_RegisterFirstOperator(uint256 quorumBitmap, uint80[] memory stakesForQuorum) public {
        ...
        uint96[] memory paddedStakesForQuorum = _registerOperatorValid(
            DEFAULT_OPERATOR,
            DEFAULT_OPERATOR_ID,
            quorumBitmap,
            stakesForQuorum
        ...
        for (uint8 i = 0; i < MAX_QUORUMS_TO_REGISTER_FOR; i++) {
            if ((quorumBitmap >> i) & 1 == 1) {
                ...
                assertEq(stakeRegistry.getLengthOfOperatorIdStakeHistoryForQuorum(DEFAULT_OPERATOR_ID, i), 1);
                // make sure that the stake update is as expected
                IStakeRegistry.OperatorStakeUpdate memory stakeUpdate = stakeRegistry
                    .getStakeUpdateForQuorumFromOperatorIdAndIndex(i, DEFAULT_OPERATOR_ID, 0);
                ...
                assertEq(stakeRegistry.getLengthOfOperatorIdStakeHistoryForQuorum(DEFAULT_OPERATOR_ID, i), 0);
                // make the analogous check for total stake history
    ...
    function testFuzz_RegisterManyOperators_Valid(
        ...
        for (uint256 i = 0; i < quorumBitmaps.length; i++) {
            (quorumBitmaps[i], paddedStakesForQuorums[i]) = _registerOperatorRandomValid(
                _incrementAddress(DEFAULT_OPERATOR, i),
                _incrementBytes32(DEFAULT_OPERATOR_ID, i),
        ...
        for (uint8 i = 0; i < MAX_QUORUMS_TO_REGISTER_FOR; i++) {
            ...
            for (uint256 j = 0; j < quorumBitmaps.length; j++) {
                ...
                if ((quorumBitmaps[j] >> i) & 1 == 1) {
                    ...
                    assertEq(
                        stakeRegistry.getLengthOfOperatorIdStakeHistoryForQuorum(
                            _incrementBytes32(DEFAULT_OPERATOR_ID, j),
                            i
                        ),
                        1
                    );
                    ...
                    // make sure that the stake update is as expected
                    IStakeRegistry.OperatorStakeUpdate memory totalStakeUpdate = stakeRegistry
                        .getTotalStakeUpdateForQuorumFromIndex(i, operatorCount);
                    ...
                    if (operatorCount != 0) {
                        IStakeRegistry.OperatorStakeUpdate memory prevTotalStakeUpdate = stakeRegistry
                            .getTotalStakeUpdateForQuorumFromIndex(i, operatorCount - 1);
    ...
    function testFuzz_DeregisterFirstOperator_Valid(
        ...
        for (uint i = 0; i < numOperatorsRegisterBefore; i++) {
            (quorumBitmaps[i], ) = _registerOperatorRandomValid(
                _incrementAddress(DEFAULT_OPERATOR, i),
                _incrementBytes32(DEFAULT_OPERATOR_ID, i),
                pseudoRandomNumber + i
            );
        ...
        uint96[] memory paddedStakesForQuorum;
        {
            ...
            paddedStakesForQuorum = _registerOperatorValid(
        ...
        {
            bool shouldPassBlockBeforeDeregistration = uint256(
                keccak256(abi.encodePacked(pseudoRandomNumber, "shouldPassBlockBeforeDeregistration"))
            ) &
                1 ==
                1;
        ...
        for (uint8 i = 0; i < MAX_QUORUMS_TO_REGISTER_FOR; i++) {
            if ((deregistrationQuroumBitmap >> i) & 1 == 1) {
                ...
                assertEq(
                    stakeRegistry.getLengthOfOperatorIdStakeHistoryForQuorum(operatorIdToDeregister, i),
                    2,
                    "testDeregisterFirstOperator_Valid_0"
                );
                // make sure that the last stake update is as expected
                IStakeRegistry.OperatorStakeUpdate memory lastStakeUpdate = stakeRegistry
                    .getStakeUpdateForQuorumFromOperatorIdAndIndex(i, operatorIdToDeregister, 1);
                assertEq(lastStakeUpdate.stake, 0, "testDeregisterFirstOperator_Valid_1");
                assertEq(
                    lastStakeUpdate.updateBlockNumber,
                    cumulativeBlockNumber,
                    "testDeregisterFirstOperator_Valid_2"
                );
                assertEq(lastStakeUpdate.nextUpdateBlockNumber, 0, "testDeregisterFirstOperator_Valid_3");

                // make the analogous check for total stake history
                assertEq(
                    stakeRegistry.getLengthOfTotalStakeHistoryForQuorum(i),
                    numOperatorsInQuorum[i] + 1,
                    "testDeregisterFirstOperator_Valid_4"
                );
                // make sure that the last stake update is as expected
                IStakeRegistry.OperatorStakeUpdate memory lastTotalStakeUpdate = stakeRegistry
                    .getTotalStakeUpdateForQuorumFromIndex(i, numOperatorsInQuorum[i]);
                assertEq(
                    lastTotalStakeUpdate.stake,
                ...
                assertEq(
                    lastTotalStakeUpdate.updateBlockNumber,
                    cumulativeBlockNumber,
                    "testDeregisterFirstOperator_Valid_6"
                );
                ...
                assertEq(
                    stakeRegistry.getLengthOfOperatorIdStakeHistoryForQuorum(operatorIdToDeregister, i),
                    1,
                    "testDeregisterFirstOperator_Valid_8"
                );
                assertEq(
                    stakeRegistry.getLengthOfTotalStakeHistoryForQuorum(i),
                    numOperatorsInQuorum[i],
                    "testDeregisterFirstOperator_Valid_9"
                );
                quorumNumberIndex++;
            } else {
                // check that the operator has 0 stake updates in the quorum numbers they did not register for
                assertEq(
                    stakeRegistry.getLengthOfOperatorIdStakeHistoryForQuorum(operatorIdToDeregister, i),
                    0,
                    "testDeregisterFirstOperator_Valid_10"
                );
            }
        }
    }

    function testFuzz_UpdateOperatorStake_Valid(uint24[] memory blocksPassed, uint96[] memory stakes) public {
        ...
        for (uint256 i = 0; i < blocksPassed.length - 1; i++) {
            IStakeRegistry.OperatorStakeUpdate memory operatorStakeUpdate = stakeRegistry
                .getStakeUpdateForQuorumFromOperatorIdAndIndex(DEFAULT_QUORUM_NUMBER, DEFAULT_OPERATOR_ID, i);
        ...
        // make sure that the last stake update is as expected
        IStakeRegistry.OperatorStakeUpdate memory lastOperatorStakeUpdate = stakeRegistry
            .getStakeUpdateForQuorumFromOperatorIdAndIndex(
                DEFAULT_QUORUM_NUMBER,
                DEFAULT_OPERATOR_ID,
                blocksPassed.length - 1
            );
    ...
    function testFuzz_RecordTotalStakeUpdate_Valid(uint24[] memory blocksPassed, uint96[] memory stakes) public {
        vm.assume(blocksPassed.length > 0);
        vm.assume(blocksPassed.length <= stakes.length);
        // initialize at a non-zero block number
        uint32 intialBlockNumber = 100;
        ...
        for (uint256 i = 0; i < blocksPassed.length - 1; i++) {
            IStakeRegistry.OperatorStakeUpdate memory totalStakeUpdate = stakeRegistry
                .getTotalStakeUpdateForQuorumFromIndex(DEFAULT_QUORUM_NUMBER, i);
    ...
    function _registerOperatorRandomValid(
        ...
        uint256 quorumBitmap = (uint256(keccak256(abi.encodePacked(psuedoRandomNumber, "quorumBitmap"))) &
            (1 << (MAX_QUORUMS_TO_REGISTER_FOR - 1))) | 1;
    ...
    function _deregisterOperatorValid(bytes32 operatorId, uint256 quorumBitmap) internal {
        bytes memory quorumNumbers = BitmapUtils.bitmapToBytesArray(quorumBitmap);
