...
contract RewardsCoordinatorUnitsTests_setOperatorSetPerformanceSplit is RewardsCoordinatorUnitTests {
    ...
    function testFuzz_Revert_WhenPaused(address operator, uint16 split) public filterFuzzedAddressInputs(operator) {
        ...
        cheats.prank(operator);
        cheats.expectRevert(IPausable.CurrentlyPaused.selector);
        rewardsCoordinator.setOperatorSetPerformanceSplit(operator, operatorSet, split);
    }
    ...
    function testFuzz_setOperatorSetPerformanceSplit(address operator, uint16 split) public filterFuzzedAddressInputs(operator) {
        cheats.assume(operator != address(0));

        split = uint16(bound(split, 0, ONE_HUNDRED_IN_BIPS));
        uint32 activatedAt = uint32(block.timestamp) + activationDelay;
        uint16 oldSplit = rewardsCoordinator.getOperatorSetPerformanceSplit(operator, operatorSet);

        cheats.expectEmit(true, true, true, true, address(rewardsCoordinator));
        emit OperatorSetPerformanceSplitBipsSet(operator, operator, operatorSet, activatedAt, oldSplit, split);
        cheats.prank(operator);
        rewardsCoordinator.setOperatorSetPerformanceSplit(operator, operatorSet, split);

        assertEq(oldSplit, rewardsCoordinator.getOperatorSetPerformanceSplit(operator, operatorSet), "Incorrect Operator split");
        cheats.warp(activatedAt);
        assertEq(split, rewardsCoordinator.getOperatorSetPerformanceSplit(operator, operatorSet), "Incorrect Operator split");
    }

    function testFuzz_setOperatorSetPerformanceSplit_UAM(address operator, uint16 split) public filterFuzzedAddressInputs(operator) {
        cheats.assume(operator != address(0));
        ...
        permissionController.setAppointee(
            ...
            defaultAppointee, 
            address(rewardsCoordinator), 
            IRewardsCoordinator.setOperatorSetPerformanceSplit.selector
        );
        ...
        cheats.expectEmit(true, true, true, true, address(rewardsCoordinator));
        emit OperatorSetPerformanceSplitBipsSet(defaultAppointee, operator, operatorSet, activatedAt, oldSplit, split);
        cheats.prank(defaultAppointee);
        rewardsCoordinator.setOperatorSetPerformanceSplit(operator, operatorSet, split);
    ...
    // Testing that the split has been initialized for the first time.
    function testFuzz_setOperatorSetPerformanceSplitFirstTime(
        address operator,
        uint16 split
    ) public filterFuzzedAddressInputs(operator) {
        cheats.assume(operator != address(0));
    ...
    // Testing the split setting for a second time prior to the earlier activation.
    function testFuzz_Revert_setOperatorSetPerformanceSplitSecondTimeBeforePriorActivation(
        address operator,
        uint16 firstSplit,
        uint16 secondSplit,
        uint32 warpTime
        ...
        // Setting First Split
        cheats.prank(operator);
        rewardsCoordinator.setOperatorSetPerformanceSplit(operator, operatorSet, firstSplit);
        // Warping to time before activation of First split
        cheats.warp(warpTime);
        ...
        cheats.prank(operator);
        cheats.expectRevert(PreviousSplitPending.selector);
        rewardsCoordinator.setOperatorSetPerformanceSplit(operator, operatorSet, secondSplit);
    }

    // Testing the split setting for a second time after earlier activation.
    function testFuzz_setOperatorSetPerformanceSplitSecondTimeAfterPriorActivation(
        address operator,
        uint16 firstSplit,
        uint16 secondSplit,
        uint32 warpTime
        ...
        // Setting Second Split
        cheats.expectEmit(true, true, true, true, address(rewardsCoordinator));
        emit OperatorSetPerformanceSplitBipsSet(operator, operator, operatorSet, activatedAt, firstSplit, secondSplit);
        cheats.prank(operator);
        rewardsCoordinator.setOperatorSetPerformanceSplit(operator, operatorSet, secondSplit);

        assertEq(firstSplit, rewardsCoordinator.getOperatorSetPerformanceSplit(operator, operatorSet), "Incorrect Operator split");
        cheats.warp(activatedAt);
        assertEq(secondSplit, rewardsCoordinator.getOperatorSetPerformanceSplit(operator, operatorSet), "Incorrect Operator split");
    }
}
...
contract RewardsCoordinatorUnitTests_createOperatorSetPerformanceRewardsSubmission is RewardsCoordinatorUnitTests {
    OperatorSet operatorSet;
    // used for stack too deep
    ...
    function test_Revert_WhenPaused() public {
        cheats.prank(pauser);
        rewardsCoordinator.pause(2 ** PAUSED_OPERATOR_SET_PERFORMANCE_REWARDS_SUBMISSION);

        cheats.expectRevert(IPausable.CurrentlyPaused.selector);
        OperatorDirectedRewardsSubmission[] memory performanceRewardsSubmissions;
        rewardsCoordinator.createOperatorSetPerformanceRewardsSubmission(operatorSet, performanceRewardsSubmissions);
    }
    ...
    //     uint256 msgValueToUse = 0;
    //     bytes memory calldataToUse = abi.encodeWithSelector(
    //         RewardsCoordinator.createOperatorSetPerformanceRewardsSubmission.selector,
    //         address(reenterer),
    ...
    //     cheats.prank(address(reenterer));
    //     cheats.expectRevert("ReentrancyGuard: reentrant call");
    //     rewardsCoordinator.createOperatorSetPerformanceRewardsSubmission(
    //         address(reenterer),
    //         performanceRewardsSubmissions
    //     );
    // }
    ...
    function testFuzz_Revert_WhenEmptyStratsAndMultipliers(
        ...
        // 3. call createOperatorSetPerformanceRewardsSubmission() with expected revert
        cheats.prank(avs);
        cheats.expectRevert(InputArrayLengthZero.selector);
        rewardsCoordinator.createOperatorSetPerformanceRewardsSubmission(operatorSet, performanceRewardsSubmissions);
    }
     ...
     * - token balance before and after of avs and rewardsCoordinator
     */
    function testFuzz_createOperatorSetPerformanceRewardsSubmission_SingleSubmission(
        address avs,
        uint256 startTimestamp,
        uint256 duration
    ) public filterFuzzedAddressInputs(avs) {
        ...
        // 3. call createOperatorSetPerformanceRewardsSubmission() with expected event emitted
        uint256 avsBalanceBefore = rewardToken.balanceOf(avs);
        uint256 rewardsCoordinatorBalanceBefore = rewardToken.balanceOf(address(rewardsCoordinator));
        ...
        emit OperatorDirectedOperatorSetRewardsSubmissionCreated(
            avs, operatorSet, currSubmissionNonce, rewardsSubmissionHash, performanceRewardsSubmissions[0]
        );
        rewardsCoordinator.createOperatorSetPerformanceRewardsSubmission(operatorSet, performanceRewardsSubmissions);
        cheats.stopPrank();
     ...
     * - token balance before and after of avs and rewardsCoordinator
     */
    function testFuzz_createOperatorSetPerformanceRewardsSubmission_SingleSubmission_UAM(
        address avs,
        uint256 startTimestamp,
        uint256 duration
    ) public filterFuzzedAddressInputs(avs) {
        ...
        permissionController.setAppointee(
            ...
            defaultAppointee,
            address(rewardsCoordinator),
            IRewardsCoordinator.createOperatorSetPerformanceRewardsSubmission.selector
        );
        ...
        cheats.expectEmit(true, true, true, true, address(rewardsCoordinator));
        emit OperatorDirectedOperatorSetRewardsSubmissionCreated(
            defaultAppointee, operatorSet, currSubmissionNonce, rewardsSubmissionHash, performanceRewardsSubmissions[0]
        );
        rewardsCoordinator.createOperatorSetPerformanceRewardsSubmission(operatorSet, performanceRewardsSubmissions);
        cheats.stopPrank();
     ...
     * - token balance before and after of avs and rewardsCoordinator
     */
    function testFuzz_createOperatorSetPerformanceRewardsSubmission_MultipleSubmissions(
        FuzzOperatorDirectedAVSRewardsSubmission memory param,
        uint256 numSubmissions
    ) public filterFuzzedAddressInputs(param.avs) {
        cheats.assume(2 <= numSubmissions && numSubmissions <= 10);
        ...
        for (uint256 i = 0; i < numSubmissions; ++i) {
            ...
            cheats.expectEmit(true, true, true, true, address(rewardsCoordinator));
            emit OperatorDirectedOperatorSetRewardsSubmissionCreated(
                param.avs, operatorSet, startSubmissionNonce + i, rewardsSubmissionHashes[i], rewardsSubmissions[i]
            );
        }

        // 4. call createAVSRewardsSubmission()
        cheats.prank(param.avs);
        rewardsCoordinator.createOperatorSetPerformanceRewardsSubmission(operatorSet, rewardsSubmissions);
