...
contract EigenPodManagerUnitTests is EigenLayerUnitTestSetup {
    ...
    function setUp() public virtual override {
        ...
        eigenPodManagerImplementation =
            new EigenPodManager(ethPOSMock, eigenPodBeacon, strategyManagerMock, slasherMock, delegationManagerMock);
            ...
            address(
                new TransparentUpgradeableProxy(
                    ...
                    abi.encodeWithSelector(
                        EigenPodManager.initialize.selector, initialOwner, pauserRegistry, 0 /*initialPausedStatus*/
                    )
                )
            )
        );
    ...
    function _deployAndReturnEigenPodForStaker(
        address staker
    ) internal returns (IEigenPod deployedPod) {
...
contract EigenPodManagerUnitTests_ShareUpdateTests is EigenPodManagerUnitTests {
    ...
    function testFuzz_addShares_revert_sharesNegative(
        int256 shares
    ) public {
    ...
    function testFuzz_addShares_revert_sharesNotWholeGwei(
        uint256 shares
    ) public {
    ...
    function testFuzz_removeShares_revert_sharesNegative(
        int256 shares
    ) public {
        cheats.assume(shares < 0);
        cheats.prank(address(delegationManagerMock));
    ...
    function testFuzz_removeShares_revert_sharesNotWholeGwei(
        uint256 shares
    ) public {
        cheats.assume(int256(shares) >= 0);
        cheats.assume(shares % GWEI_TO_WEI != 0);
    ...
    function testFuzz_withdrawSharesAsTokens_revert_sharesNegative(
        int256 shares
    ) public {
        cheats.assume(shares < 0);
        cheats.prank(address(delegationManagerMock));
    ...
    function testFuzz_withdrawSharesAsTokens_revert_sharesNotWholeGwei(
        uint256 shares
    ) public {
        cheats.assume(int256(shares) >= 0);
        cheats.assume(shares % GWEI_TO_WEI != 0);
...
contract EigenPodManagerUnitTests_BeaconChainETHBalanceUpdateTests is
    EigenPodManagerUnitTests,
...
contract EigenPodManagerUnitTests_ShareAdjustmentCalculationTests is EigenPodManagerUnitTests {
    ...
    function setUp() public virtual override {
        ...
        eigenPodManagerWrapper = new EigenPodManagerWrapper(
            ethPOSMock, eigenPodBeacon, strategyManagerMock, slasherMock, delegationManagerMock
        );
        eigenLayerProxyAdmin.upgrade(
            TransparentUpgradeableProxy(payable(address(eigenPodManager))), address(eigenPodManagerWrapper)
        );
    }
