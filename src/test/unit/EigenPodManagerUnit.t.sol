...
contract EigenPodManagerUnitTests is EigenLayerUnitTestSetup, IEigenPodManagerEvents {
    ...
    function _initializePodWithShares(address podOwner, int shares) internal {
        ...
        if (shares >= 0) {
            ...
            eigenPodManager.addShares(podOwner, beaconChainETHStrategy, IERC20(address(0)), uint(shares));
        } else {
...
contract EigenPodManagerUnitTests_ShareUpdateTests is EigenPodManagerUnitTests {
    ...
    function testFuzz_addShares_revert_sharesNegative(
        ...
        eigenPodManager.addShares(defaultStaker, beaconChainETHStrategy, IERC20(address(0)), uint(shares));
    }
     ...
     *
     */
    function testFuzz_removeDepositShares_revert_notDelegationManager(
        address notDelegationManager
    ) public filterFuzzedAddressInputs(notDelegationManager) {
        cheats.assume(notDelegationManager != address(delegationManagerMock));
        cheats.prank(notDelegationManager);
    ...
    function testFuzz_removeDepositShares_revert_sharesNegative(
        uint224 sharesToRemove
    ) public {
...
contract EigenPodManagerUnitTests_BeaconChainETHBalanceUpdateTests is EigenPodManagerUnitTests {
    ...
    function testFuzz_revert_notPod(
    ...
    function testFuzz_revert_nonWholeGweiAmount(
        int sharesDelta
    ...
    function testFuzz_revert_negativeDepositShares(
        int224 sharesBefore
    ) public {
