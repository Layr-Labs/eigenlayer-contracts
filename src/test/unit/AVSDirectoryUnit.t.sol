...
import "src/contracts/libraries/ShareScalingLib.sol";
...
contract AVSDirectoryUnitTests is EigenLayerUnitTestSetup, IAVSDirectoryEvents {
    ...
    function _deployMockStrategy(address underlyingToken) internal returns (IStrategy) {
        return StrategyBase(
            address(
                new TransparentUpgradeableProxy(
                    address(new StrategyBase(strategyManagerMock)),
...
contract AVSDirectoryUnitTests_registerOperatorToOperatorSet is AVSDirectoryUnitTests {
    ...
    function testFuzz_Correctness_MultipleSets(
        ...
        for (uint32 i = 1; i < totalSets + 1; ++i) {
            IAVSDirectory.OperatorSet memory operatorSet = IAVSDirectory.OperatorSet(address(this), i);
            assertTrue(avsDirectory.isMember(operator, operatorSet));
            assertEq(avsDirectory.getNumOperatorsInOperatorSet(operatorSet), 1);
...
contract AVSDirectoryUnitTests_modifyAllocations is AVSDirectoryUnitTests {
    function test_sanity_modifyAllocations_CorrectInitialAllocatableMagnitude() public {
        address operator = address(0xCAFE);

        _registerOperatorWithBaseDetails(operator);

        assertEq(
            avsDirectory.getAllocatableMagnitude(operator, IStrategy(address(this)), uint8(0)),
            ShareScalingLib.INITIAL_TOTAL_MAGNITUDE
        );
    }
    ...
    function test_revert_modifyAllocations_OperatorNotRegistered() public {
        ...
        ERC20PresetFixedSupply underlyingToken = new ERC20PresetFixedSupply("Test Token", "TEST", 100 ether, operator);
        ...
        allocations[0] = IAVSDirectory.MagnitudeAllocation({
            strategy: IStrategy(address(this)),
            expectedTotalMagnitude: 0,
            operatorSets: new IAVSDirectory.OperatorSet[](0),
    ...
    function test_revert_modifyAllocations_NonexistantOperatorSet() public {
        uint64 max = 1e18;
        address operator = address(0xCAFE);
        ...
        cheats.prank(operator);
        cheats.expectRevert("AVSDirectory._modifyAllocations: operatorSet does not exist");
        avsDirectory.modifyAllocations(
            operator,
            allocations,
            ISignatureUtils.SignatureWithSaltAndExpiry(new bytes(65), bytes32(0), block.timestamp)
    ...
    function test_revert_modifyAllocations_AllocationExeedsMax() public {
        uint64 max = 1e18;
        address operator = address(0xCAFE);
        ...
        cheats.prank(operator);
        cheats.expectRevert("AVSDirectory._modifyAllocations: insufficient available free magnitude to allocate");
        avsDirectory.modifyAllocations(
            operator,
            allocations,
            ISignatureUtils.SignatureWithSaltAndExpiry(new bytes(65), bytes32(0), block.timestamp)
    ...
    function test_revert_modifyAllocations_MultiplePendingAllocations() public {
        uint64 max = 1e18;
        address operator = address(0xCAFE);
    ...
    function test_revert_modifyAllocations_InputArrayLengthMismatch() public {
        uint64 max = 1e18;
        address operator = address(0xCAFE);
        ...
        cheats.prank(operator);
        cheats.expectRevert("AVSDirectory._modifyAllocations: operatorSets and magnitudes length mismatch");
        avsDirectory.modifyAllocations(
            operator,
            allocations,
            ISignatureUtils.SignatureWithSaltAndExpiry(new bytes(65), bytes32(0), block.timestamp)
