 ...
 * and designed specifically for use with 'fee-on-transfer' tyoken.
 ...
 * @dev This contract is intended for use with 'fee-on-transfer'-type tokens, although it should also support feeless tokens.
 * @dev Note that this contract will *not* work properly with 'rebasing'-type tokens. If the `underlyingToken` is set to a rebasing-type
 * token, then each new deposit will be credited with the increase in this contract's balance that has occurred (especially as a result
 * of rebasing) since the last deposit or withdrawal.
 * @dev TODO: properly test this Strategy with different fee-on-transfer tokens
 */
contract FeeOnTransfer_StrategyBase is Initializable, Pausable, IStrategy {
    ...
    */
    ...
    /// @notice The stored balance of underlying tokens
        ...
        returns (uint256 newShares)
    {
        ...
        if (totalShares == 0) {
            ...
            uint256 priorTokenBalance = tokenReserves;
