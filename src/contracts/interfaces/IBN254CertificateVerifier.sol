...
interface IBN254CertificateVerifierErrors {
    ...
    /// @dev 4byte error code: 0x03f4a78e
    /// @dev We enforce that operator indices are valid to ensure proper merkle proof verification and prevent out-of-bounds access to the operator set
    ...
    IBN254CertificateVerifierErrors
{
     ...
     * @dev Reverts for:
     * - OnlyTableUpdater: caller is not the operator table updater
     ...
     * @dev Reverts for:
     * - CertificateStale: certificate's referenceTimestamp is too stale per maxStalenessPeriod
     * - ReferenceTimestampDoesNotExist: the reference timestamp has not been set for the operatorSet
     * - RootDisabled: the root at the referenceTimestamp is disabled
     * - InvalidOperatorIndex: operator index in nonSignerWitnesses is invalid
     ...
     * - InvalidOperatorIndex: operator index in nonSignerWitnesses is invalid
     * - VerificationFailed: merkle proof verification failed or BLS signature verification failed
     * - ArrayLengthMismatch: signedStakes.length != totalStakeProportionThresholds.length
     */
     ...
     * - InvalidOperatorIndex: operator index in nonSignerWitnesses is invalid
     * - VerificationFailed: merkle proof verification failed or BLS signature verification failed
     * - ArrayLengthMismatch: signedStakes.length != totalStakeNominalThresholds.length
     */
