    ...
    ReentrancyGuardUpgradeable
{
    ...
    function modifyAllocations(
        ...
        for (uint256 i = 0; i < allocations.length; ++i) {
            ...
            _clearModificationQueues({operator: msg.sender, strategy: allocation.strategy, numToClear: type(uint16).max});
            ...
            for (uint256 j = 0; j < allocation.operatorSets.length; ++j) {
                ...
                if (info.pendingDiff < 0) {
                    ...
                    // Add the operatorSet to the allocation queue
                    allocationQueue[msg.sender][allocation.strategy].pushBack(operatorSetKey);
                }
    ...
    function clearModificationQueues(
        ...
        for (uint256 i = 0; i < strategies.length; ++i) {
            _clearModificationQueues({operator: operator, strategy: strategies[i], numToClear: numToClear[i]});
        }
    }
     ...
     */
    function _clearModificationQueues(address operator, IStrategy strategy, uint16 numToClear) internal {
        uint256 numCompleted;
        uint256 length = deallocationQueue[operator][strategy].length();

        // Clear deallocations first
        while (length > 0 && numCompleted < numToClear) {
            ...
            // we can stop. Any subsequent deallocations will also be uncompletable.
        ...
        // Clear for allocations now
        length = allocationQueue[operator][strategy].length();
        while (length > 0 && numCompleted < numToClear) {
            bytes32 operatorSetKey = allocationQueue[operator][strategy].front();
            PendingMagnitudeInfo memory info = _getPendingMagnitudeInfo(operator, strategy, operatorSetKey);

            // If we've reached a pending allocation that isn't completable yet,
            // we can stop. Any subsequent allocations will also be uncompletable.
            if (block.timestamp < info.effectTimestamp) {
                break;
            }
            ...
            // Remove the allocation from the queue
            allocationQueue[operator][strategy].popFront();
            ++numCompleted;
            --length;
        }
    }
    ...
    function _setAllocationDelay(address operator, uint32 delay) internal {
        ...
        require(delay < ALLOCATION_CONFIGURATION_DELAY, InvalidAllocationDelay());
    ...
    function getAllocatableMagnitude(address operator, IStrategy strategy) external view returns (uint64) {
        // This method needs to simulate clearing any pending allocation/deallocation modifications.
        // This roughly mimics the calculations done in `_clearModificationQueues` and
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            // Add to encumbered magnitude. Allocations
