...
abstract contract AVSDirectoryStorage is IAVSDirectory {
    ...
    bytes32 public constant OPERATOR_STANDBY_UPDATE =
        keccak256("OperatorStandbyUpdate(address avs,uint32 id,bool onStandby,bytes32 salt,uint256 expiry)");
    ...
    /// @notice Mapping: avs = operatorSetId => Whether the given operator set in standby mode or not.
    mapping(address => mapping(uint32 => bool)) public onStandby;
